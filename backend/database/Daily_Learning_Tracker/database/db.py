...
Provides:
  ...
  • get_detection_case_full()         — full case bundle (all pipeline tables)
  • get_dashboard_stats()             — aggregate counts for the dashboard
  ─── detections (pipeline) ───────────────────────────────────
  • add_detection()                   — record a per-engine signal
  • get_detections_for_case()         — retrieve all signals for a case
  ─── platform_checks (pipeline) ──────────────────────────────
  • save_platform_check()             — save URL/domain analysis result
  • get_platform_check()              — retrieve platform check for a case
  ─── risk_scores (pipeline) ──────────────────────────────────
  • save_risk_score()                 — upsert Risk Engine output
  • get_risk_score()                  — retrieve risk score for a case
  ─── reports (pipeline) ──────────────────────────────────────
  • save_report()                     — upsert investigation report
  • get_report()                      — retrieve report for a case
  ─── alerts ──────────────────────────────────────────────────
  • create_alert()                    — insert an alert for a case
  • list_unread_alerts()              — unread alerts (notification badge)
  ...
  ─── evidence ────────────────────────────────────────────────
  • add_evidence()                    — attach evidence to a case
  ─── LEGACY (kept for backwards compatibility) ───────────────
  • create_case()  • update_case_status()  • list_cases()
  • add_signal()   • save_result()         • add_log()
  • get_case_with_details()

Environment variables (see .env.example):
    SUPABASE_URL  — Supabase project URL
    SUPABASE_KEY  — anon/public key  (use service_role key on the backend)
"""
...
# ===========================================================================
# PIPELINE HELPERS — New tables added in migration 002
# All functions follow the same patterns as the existing helpers above.
# ===========================================================================
...
def add_detection(
    ...
    """
    Record an individual detection signal from a pipeline engine.

    Args:
        case_id:        UUID of the parent detection_cases row.
        detection_type: 'deepfake' | 'audio' | 'fraud_language' |
                        'impersonation' | 'platform' | 'url' | 'other'
        score:          Normalised signal score 0.0-1.0.
        confidence:     Model confidence 0.0-1.0.
        label:          Prediction label e.g. 'DEEPFAKE', 'REAL', 'SUSPICIOUS'.
        signals:        Full engine output dict (per-frame scores, phrases, etc.)
        evidence:       Dict referencing evidence items used.

    Returns:
        The created detections row.
    """
    payload: dict[str, Any] = {
        "case_id":        case_id,
    ...
    resp = get_client().table("detections").insert(payload).execute()
    return resp.data[0]


def get_detections_for_case(
    case_id: str,
    detection_type: Optional[str] = None,
) -> list[dict[str, Any]]:
    """
    Return all detection signals for a case, optionally filtered by type.

    Args:
        case_id:        UUID of the detection case.
        detection_type: Optional filter (e.g. 'deepfake').

    Returns:
        List of detections rows ordered by created_at.
    """
    q = (
        get_client()
        .table("detections")
    ...
    if detection_type is not None:
        q = q.eq("detection_type", detection_type)
    return q.execute().data
...
def save_platform_check(
    case_id: str,
    url: Optional[str] = None,
    domain: Optional[str] = None,
    api_score: Optional[float] = None,
    local_check_score: Optional[float] = None,
    final_score: Optional[float] = None,
    signals: Optional[dict[str, Any]] = None,
) -> dict[str, Any]:
    """
    Persist URL/domain platform analysis from the external API + local heuristics.

    Args:
        case_id:           UUID of the parent detection case.
        url:               Full URL that was analysed.
        domain:            Extracted domain.
        api_score:         Score from external threat-intel API (0.0-1.0); None if unavailable.
        local_check_score: Score from local heuristics (0.0-1.0).
        final_score:       Weighted combined score (0.0-1.0) sent to the Risk Engine.
        signals:           Full structured analysis output dict.

    Returns:
        The created platform_checks row.
    """
    ...
    if api_score is not None:
        payload["api_score"] = round(float(api_score), 4)
    if local_check_score is not None:
        payload["local_check_score"] = round(float(local_check_score), 4)
    if final_score is not None:
        payload["final_score"] = round(float(final_score), 4)
    if signals is not None:
        payload["signals"] = signals

    resp = get_client().table("platform_checks").insert(payload).execute()
    return resp.data[0]


def get_platform_check(case_id: str) -> Optional[dict[str, Any]]:
    """Return the platform check for a case, or None if not run."""
    resp = (
        get_client()
...
def save_risk_score(
    ...
    fraud_language_score: Optional[float] = None,
    impersonation_score: Optional[float] = None,
    platform_score: Optional[float] = None,
) -> dict[str, Any]:
    """
    Persist the Risk Engine / Signal Fusion output for a case.
    Uses upsert (UNIQUE on case_id) so re-running the engine is safe.

    Also updates the detection_cases denormalised summary fields
    (risk_score, risk_level) to keep dashboard queries fast.

    Args:
        case_id:              UUID of the detection case.
        overall_score:        Weighted aggregate 0-100.
        risk_level:           'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'
        explanation:          Explanation Engine dict (why_flagged, top_signals, etc.)
        deepfake_score:       Component score (0-100); None if engine did not run.
        audio_score:          Component score; None if not run.
        fraud_language_score: Component score; None if not run.
        impersonation_score:  Component score; None if not run.
        platform_score:       Component score; None if not run.

    Returns:
        The created/updated risk_scores row.
    """
    payload: dict[str, Any] = {
        "case_id":       case_id,
    ...
    if deepfake_score is not None:
        payload["deepfake_score"] = round(float(deepfake_score), 2)
    if audio_score is not None:
        payload["audio_score"] = round(float(audio_score), 2)
    if fraud_language_score is not None:
        payload["fraud_language_score"] = round(float(fraud_language_score), 2)
    if impersonation_score is not None:
        payload["impersonation_score"] = round(float(impersonation_score), 2)
    if platform_score is not None:
        payload["platform_score"] = round(float(platform_score), 2)
    ...
    # Keep detection_cases in sync (denormalised cache)
    get_client().table("detection_cases").update({
        "risk_score": round(float(overall_score), 2),
        "risk_level": risk_level,
...
def get_risk_score(case_id: str) -> Optional[dict[str, Any]]:
    """Return the risk score row for a case, or None if not yet computed."""
    resp = (
        get_client()
        .table("risk_scores")
        .select("*")
...
def save_report(
    ...
    """
    Persist the final investigation report for a case.
    Uses upsert (UNIQUE on case_id) so regenerating is safe.

    Args:
        case_id:         UUID of the detection case.
        summary:         Brief paragraph explanation of findings.
        risk_level:      'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'
        recommendations: JSON dict with 'actions' list and 'regulatory_refs'.

    Returns:
        The created/updated reports row.
    """
    payload = {
        "case_id":         case_id,
        "summary":         summary,
...
def get_report(case_id: str) -> Optional[dict[str, Any]]:
    """Return the investigation report for a case, or None if not generated."""
    resp = (
        get_client()
        .table("reports")
        .select("*")
...
# ---------------------------------------------------------------------------
# Extended: get_detection_case_full (adds detections, risk_score, report)
# ---------------------------------------------------------------------------

def get_detection_case_full(case_id: str) -> dict[str, Any]:
    """
    Fetch a detection case with ALL related data — replaces the previous version.

    Returns:
        {
          "case":                 detection_cases row,
          "evidence":             [...],
          "detections":           [...],       # new pipeline table
          "impersonation_check":  row | None,
          "platform_check":       row | None,  # new pipeline table
          "risk_score":           row | None,  # new pipeline table
          "alerts":               [...],
          "report":               row | None,  # new pipeline table
        }
    """
    client = get_client()

    case_resp = client.table("detection_cases").select("*").eq("id", case_id).execute()
    if not case_resp.data:
        raise ValueError(f"Detection case not found: {case_id}")
    ...
    rs_resp = (
        client.table("risk_scores").select("*").eq("case_id", case_id).execute()
    )
    alerts_resp = (
        client.table("alerts")
        .select("*")
    ...
    report_resp = (
        client.table("reports").select("*").eq("case_id", case_id).execute()
    )

    return {
        "case":                case_resp.data[0],
        "evidence":            evidence_resp.data,
        "detections":          detections_resp.data,
        "impersonation_check": imp_resp.data[0] if imp_resp.data else None,
        "platform_check":      pc_resp.data[0] if pc_resp.data else None,
        "risk_score":          rs_resp.data[0] if rs_resp.data else None,
        "alerts":              alerts_resp.data,
