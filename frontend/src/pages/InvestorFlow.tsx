        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full mb-8"
            ...
            <svg width="18" height="18" viewBox="0 0 22 22" fill="none">
              <circle cx="11" cy="7.5" r="4" stroke="#4FD1C5" strokeWidth="1.7"/>
              <path d="M3 19c0-4.418 3.582-8 8-8s8 3.582 8 8" stroke="#4FD1C5" strokeWidth="1.7" strokeLinecap="round"/>
            </svg>
            <span className="text-sm font-medium" style={{ color: "#A8BDD4" }}>You are protected by SafeInvest AI</span>
          </div>

          <h1 className="font-display text-3xl lg:text-4xl font-semibold text-white leading-tight mb-4">
            Got a suspicious investment offer? Let's check it together.
          </h1>
          <p style={{ color: "#A8BDD4" }} className="text-base leading-relaxed mb-8 max-w-xl mx-auto">
            Share whatever evidence you have received — a message, a video, or a website link. Tap the card that matches and follow the simple steps. You can add one type or all three.
          </p>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-sm font-semibold" style={{ color: done ? "#16A34A" : m.color }}>
                    {done ? (
                      <><span style={{ color: "#16A34A" }}>✓</span> Added — tap to change</>
                    ) : (
                      <><span style={{ color: m.color }}>→</span> Tap to add this</>
                    )}
                  </div>
                </button>
              );
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-6 py-5 flex flex-col sm:flex-row items-center gap-5 justify-between">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <p className="text-sm text-slate-600">
                {filled === 0
                  ? "👆 Tap any card above to get started"
                  : filled === 3
                  ? "✅ All three types added — ready to analyze!"
                  : `${filled} of 3 added — you can analyze now or add more`}
              </p>
            </div>
            ...
            <button
              ...
              {filled === 0 ? "Add evidence to continue" : "Get My Risk Report →"}
            </button>
          </div>

          {filled > 0 && filled < 3 && (
            <p className="text-center text-xs text-slate-400 mt-3">
              💡 Adding more evidence types gives a more complete picture — but you can get your report now if you prefer.
            </p>
          )}
        </div>
      </section>
